...
module.exports = userConfig => {
  ...
  if (process.env.NODE_ENV !== 'development' || config.useCloud) {
    return `<figure ${config.classes ? ` class="${config.classes}"` : ''}><img src="${settings.cloudinary.url}/f_auto,q_${config.quality},w_iw${config.transforms ? `,${config.transforms}` : ''}/${settings.url}${settings.paths.images}${config.imagePath}" sizes="(min-width: 720px) 980px, 100vw"
